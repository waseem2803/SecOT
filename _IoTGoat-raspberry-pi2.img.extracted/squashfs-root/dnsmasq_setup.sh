#!/bin/sh
echo -e "#############\nPlease ensure Dnsmasq network setup has been followed. See http://bit.ly/dnsmasq."\nTo set back to default, use -d default \nUsage: sh ./dnsmasq_setup.sh\n 
echo -e "\n#############"
...
if [ -z "${default}" ]; then
   cp $netbak /etc/config/network && cp $dnsbak /etc/dnsmasq.conf && ifup lan && ifup wan; exit 1;
fi
